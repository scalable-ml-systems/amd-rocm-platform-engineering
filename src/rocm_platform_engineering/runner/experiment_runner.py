"""Minimal command runner for AMD ROCm experiments."""
...
DEFAULT_RESULTS_DIRECTORY = Path("benchmark-results/raw")
...
def create_run_id(
    ...
    """Create the standard experiment run identifier."""

    timestamp = started_at_utc.strftime("%Y%m%dT%H%M%SZ")
    return f"run-{timestamp}-{git_revision[:8]}"
...
def execute_experiment(
    ...
    """Execute one command and preserve its evidence."""
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Execute one command and capture an experiment "
            "evidence bundle."
        )
    )
    parser.add_argument(
